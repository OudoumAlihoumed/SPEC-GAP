...
Two layers:
  ...
  2. Semantic invariants that JSON Schema cannot express and that map to the
     bugs this project actually hit:
       ...
       INV-5  behavioral chain_completed and the deprecated aliases agree with
              the unsafe_action events.
...
def semantic_checks(t):
    ...
    # INV-5: behavioral chain_completed agrees with unsafe_action events.
    ...
    if el["behavioral_channel"]["chain_completed"] != chain:
        errs.append("INV-5 behavioral_channel.chain_completed disagrees with unsafe_action events")
    for alias in ("injection_success", "exfiltration_detected"):
        if alias in el and el[alias] != chain:
            errs.append(f"INV-5 deprecated alias {alias} disagrees with chain completion")
