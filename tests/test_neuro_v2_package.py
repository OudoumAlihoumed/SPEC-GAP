"""Review invariants for the publication-backed Neuro v2 package."""
...
REGISTRY_PATH = PACKAGE_ROOT / "registry_gen5000_v2.json"
PLAN_PATH = (
    PACKAGE_ROOT / "retrieval" / "full_corpus_bm25_balanced_gen5000_v2.json"
)
SUMMARY_PATH = (
    ROOT
    / "results"
...
def test_neuro_v2_provenance_and_carrier_are_unambiguous():
...
def test_neuro_v2_nc_nd_document_has_an_explicit_license_boundary():
    registry = _load_json(REGISTRY_PATH)
    neuro_doc3 = next(
        item for item in registry["provenance"]["source_documents"]
        if item["doc_id"] == "neuro_doc3"
    )
    notice = (PACKAGE_ROOT / "README.md").read_text()
...
def test_neuro_v2_summary_enumerates_the_complete_eight_condition_matrix():
